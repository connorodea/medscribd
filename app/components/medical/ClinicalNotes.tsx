...
export default function ClinicalNotes() {
  // Removed excessive logging that was showing on every render
  ...
  // Stable refs for current values to reduce re-renders
  ...
  useEffect(() => {
    console.debug('📝 ClinicalNotes: Socket status changed:', socket ? 'connected' : 'null');
    if (socket) {
      console.debug('📝 ClinicalNotes: Socket ready state:', socket.readyState);
      console.debug('📝 ClinicalNotes: Socket URL:', socket.url);
    }
  }, [socket]);
  ...
  const handleSaveNote = useCallback(async () => {
    ...
    const noteContent = currentNoteRef.current.trim();
  ...
  useEffect(() => {
    console.debug('📝 ClinicalNotes: Setting up WebSocket listener, socket:', socket ? 'connected' : 'null');
    if (!socket) return;

    const handleMessage = (event: MessageEvent) => {
      console.debug('📝 ClinicalNotes: Received WebSocket message:', event.data);
      
      if (typeof event.data !== 'string') {
        console.debug('📝 ClinicalNotes: Message data is not string, ignoring');
        return;
      }

      try {
        const message = JSON.parse(event.data);
        console.debug('📝 ClinicalNotes: Parsed message:', message);
        
        if (message.type !== 'FunctionCallRequest') {
          console.debug('📝 ClinicalNotes: Message type is not FunctionCallRequest, ignoring. Type:', message.type);
          return;
        }

        console.log('📝 ClinicalNotes: Processing FunctionCallRequest');
        const functionCall = message.functions?.[0];
        if (!functionCall) {
          console.debug('📝 ClinicalNotes: No function call found in message');
          return;
        }
        ...
        console.debug('📝 ClinicalNotes: Function details:', { function_name, function_call_id, input });
        
        let success = true;
        let formattedValue = '';
        const newFields = { ...currentFields };
        console.debug('📝 ClinicalNotes: Current fields before update:', currentFields);

        switch (function_name) {
          case 'set_patient_name':
            console.debug('📝 ClinicalNotes: Setting patient name to:', input.name);
            newFields.demographics.patientName = input.name;
            formattedValue = `Patient Name: ${input.name}\n`;
            console.debug('📝 ClinicalNotes: Updated demographics:', newFields.demographics);
            break;
          case 'set_date_of_birth':
            console.debug('📝 ClinicalNotes: Setting date of birth to:', input.dateOfBirth);
            newFields.demographics.dateOfBirth = input.dateOfBirth;
            formattedValue = `Date of Birth: ${input.dateOfBirth}\n`;
            break;
          case 'set_gender':
            console.debug('📝 ClinicalNotes: Setting gender to:', input.gender);
            newFields.demographics.gender = input.gender;
            formattedValue = `Gender: ${input.gender}\n`;
            break;
          case 'set_mrn':
            console.debug('📝 ClinicalNotes: Setting MRN to:', input.mrn);
          ...
          case 'clear_note':
            ...
            break;
          default:
            console.debug('📝 ClinicalNotes: Unknown function name:', function_name);
            success = false;
            break;
        }

        console.debug('📝 ClinicalNotes: Function processing complete. Success:', success);
        console.debug('📝 ClinicalNotes: Formatted value:', formattedValue);
        console.debug('📝 ClinicalNotes: New fields to set:', newFields);

        if (formattedValue) {
          console.debug('📝 ClinicalNotes: Updating current note with formatted value');
          setCurrentNote(prev => {
            const newNote = prev + formattedValue;
            console.debug('📝 ClinicalNotes: Current note updated from:', prev, 'to:', newNote);
            return newNote;
          });
          console.debug('📝 ClinicalNotes: Updating current fields');
          setCurrentFields(newFields);
          console.debug('📝 ClinicalNotes: Current fields updated to:', newFields);
        } else {
          console.debug('📝 ClinicalNotes: No formatted value, skipping state updates');
        }

        console.debug('📝 ClinicalNotes: Sending response back to socket');
        ...
        console.debug('📝 ClinicalNotes: Response sent:', { function_call_id, function_name, success });
      } catch (error) {
        console.error('📝 ClinicalNotes: Error handling message:', error);
      }
    };

    socket.addEventListener('message', handleMessage);
    console.debug('📝 ClinicalNotes: WebSocket message listener added');
    
    return () => {
      console.debug('📝 ClinicalNotes: WebSocket message listener removed');
