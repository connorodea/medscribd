...
export const App = ({
  ...
  useEffect(() => {
    if (microphoneState === 1 && socket && defaultStsConfig) {
      ...
      const onOpen = () => {
        ...
        console.debug("=== SENDING SETTINGS MESSAGE ===");
        console.debug(JSON.stringify(combinedStsConfig, null, 2));
        console.debug("=== END SETTINGS MESSAGE ===");
  ...
  const onMessage = useCallback(
    async (event) => {
      if (event.data instanceof ArrayBuffer) {
        ...
        console.debug("=== INCOMING WEBSOCKET MESSAGE ===");
        ...
        try {
          ...
          if (parsedMessage.type === "SpeakUpdated") {
            console.debug("🎉 === SPEAK UPDATED MESSAGE RECEIVED ===");
            console.debug("Voice change was successful!");
          } else if (parsedMessage.type === "Error") {
            console.debug("❌ === ERROR MESSAGE RECEIVED ===");
            console.debug("Error details:", parsedMessage);
          } else {
            console.debug("📨 Message type:", parsedMessage.type);
          }
        } catch (e) {
          console.debug("Could not parse message as JSON:", e);
        }
  ...
  // Listen for voice change events and send UpdateSpeak
  useEffect(() => {
    const handleVoiceChanged = (event) => {
      const newVoice = event.detail.voice;
      
      // Always send UpdateSpeak for any voice change (bypasses multiple hook instances issue)
      if (newVoice && socket && socketState === 1) {
