...
export function VoiceBotProvider({ children }: Props) {
  ...
  useEffect(() => {
    // Only run sleep timer when bot is listening (not when sleeping, speaking, etc.)
    if (state.status === VoiceBotStatus.LISTENING) {
    ...
    return;
  }, [state.status]); // Only restart timer when status changes
